...
def is_sentry_signature_correct(body: Mapping[str, Any], key: str, expected: str) -> bool:
    # digest = hmac.new(
...
def handle_incoming(request):
    ...
    if not is_sentry_signature_correct(raw_body, SENTRY_CLIENT_SECRET, request.headers.get("sentry-hook-signature")):
        return Response("", 401)
...
def _handle_alert(resource, action, data):
    return Response("", 200)


def _handle_error(resource, action, data):
...
def handle_sentry_incoming(resource, action, data):
    try:
